...
4. **Explicit pass/fail signal** — each AC has an observable, deterministic outcome: a screen reached, an exact message string, or an element state. No "works", "looks good", "is correct". Gap → ask for the concrete signal (the user, the ticket, or a doc supplies it). Don't harvest selectors/strings from a running app — `/from-issue` confirms those at generation time.
   ...
   **A value on screen is not always a value a test can read.** Colours, images, chart shapes and anything rendered without text or an accessible label may not appear in the element tree at all — on mobile this is common, and the AC is written before anyone checks. When an AC asks for such a value, say so in the assumptions and offer the checkable version ("the row shows a colour swatch" rather than "shows its colour"), so the author decides rather than `/from-issue` quietly weakening the assertion. Real case: OR-2's AC 7 asked a cart row to show "its colour"; the swatch is an image with a generic description and the colour itself is unreadable.
